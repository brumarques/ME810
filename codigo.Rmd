...
library(knitr)
library(xtable)
data <- read_excel("~/ME810-master/data.xlsx") #leitura dos dados
...
```{r,echo=FALSE }
fit = lm(Concentração~Área, data=data)
plot(Concentração~Área, data=data, main="Curva Analítica")
abline(fit, col="red")

```

Acima, temos a reta de regressão (curva analítica) traçada em cima dos pontos. A reta encontrada foi:   
$$Y = `r round(coefficients(fit)[2], 6)`X + `r round(coefficients(fit)[1], 3)`$$
